...
public class SampleRepository {
    ...
    public Optional<SampleEntity> findFirst(boolean followerRead) {
        return jdbcTemplate
                .query(followerRead ? "SELECT * FROM pooltool_test "
                                      + "as of system time follower_read_timestamp() order by id limit 1" :
                                "SELECT * FROM pooltool_test order by id limit 1",
                        entityRowMapper())
                .stream()
                .findFirst();
    }
    ...
    public Optional<SampleEntity> findByNextId(UUID id, boolean followerRead) {
        return jdbcTemplate
                .query(followerRead ? "SELECT * FROM pooltool_test "
                                ...
                                "SELECT * FROM pooltool_test where id > ? order by id limit 1",
                        entityRowMapper(), id)
                .stream()
                .findFirst();
    }
    ...
    public Optional<SampleEntity> findRandom() {
        return jdbcTemplate
                .query("SELECT * FROM pooltool_test ORDER BY random() limit 1",
                        entityRowMapper())
                .stream()
                .findFirst();
    }
