...
public class JdbcSampleRepository implements SampleRepository {
    ...
    public List<SampleEntity> insertBatch(int batchSize) {
        ...
        jdbcTemplate.update(
                "INSERT INTO hibachi_test (id,expire_at,payload,version) "
                + "select unnest(?) as id, "
                + "       unnest(?) as expire_at, "
                + "       unnest(?) as payload, "
                + "       unnest(?) as version",
    ...
    public void updateSingleton(SampleEntity entity) {
        final String sql = "UPDATE hibachi_test SET expire_at=?,payload=?,version=? "
    ...
    public void deleteById(UUID id) {
        jdbcTemplate.update("DELETE from hibachi_test WHERE id=? and version=0",
                ps -> ps.setObject(1, id));
    }

    @Override
    public List<SampleEntity> findAll(int limit) {
        return jdbcTemplate.query("SELECT * FROM hibachi_test limit " + limit,
                entityRowMapper());
    }
    ...
    public void deleteAll() {
        jdbcTemplate.execute("delete from hibachi_test where 1=1");
    }
